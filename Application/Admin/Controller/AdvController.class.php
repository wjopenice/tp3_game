...
class AdvController extends ThinkController {
    ...
    */
    ...
    /**
    *编辑广告位
    * @param $back 默认0：U方法 1 完整地址
    */
    protected function baes_edit($model="",$id=0,$page_url="",$back=0){
        //获取模型信息
        ...
        if(IS_POST){
            ...
            if($Model->create() && $Model->save()){
                if($back == 1){
                     $this->success('保存'.$model['title'].'成功！', $page_url);
                }else{
    ...
    */
    public function edit_adv($model="",$id=0){
        ...
        $this->baes_edit($model,$ids,session('back_url'),1);
    }
