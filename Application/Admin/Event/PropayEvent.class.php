...
class PropayEvent extends Controller {
    ...
    */
    public function add1(){
      ...
      $amount= $_POST['amount'];
      $pro=$this->isset_promote($promote_account);
      if($pro!==null&&$pro['parent_id']==0){
    ...
    public function add3(){
        ...
        for($i=0;$i<count($namearr);$i++){
            ...
            if(null!=$user&&$user['parent_id']==0){
                $add['promote_id']=$user['id'];
                $add['promote_account']=$user['account'];
                $add['order_number']=build_order_no();
                $add['amount']=$amount;
                $add['status']=0;
                $add['admin_id']=UID;
        ...
        $b=count($namearr)-$a;
        $this->success("成功{$a}个,失败{$b}个",U("propay"));
    }
    ...
    public function save_import($data){ 
        ...
        foreach ($data as $k=>$v){
          ...
          if($v['C']<=0){//金额有问题
            ...
            $proo=$this->isset_promote($v['A']);
          if($proo==null){//用户不存在
            $errorList[$errorNum]['D'] = '用户不存在';
            $errorNum++;
            continue;
          }
          if($proo!==null&&$proo['parent_id']!=0){
            $errorList[$errorNum]['D'] = '请发放到渠道主账户';
            $errorNum++;
            continue;
          }
          $succNum++;
