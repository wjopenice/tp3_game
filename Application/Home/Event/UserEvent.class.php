...
class UserEvent extends BaseEvent {
    ...
    public function join_list($model,$p){
        ...
        $real_sum=M($name,'tab_')->field('sum(real_money) as sum')->where(array("status"=>1))->find();
         //分页
          $n_real_sum=M($name,'tab_')->field('sum(real_money) as sum')->where(array("status"=>0))->find();
        if($count > $row){
            $page = new \Think\Page($count, $row);
            $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
            $this->assign('_page', $page->show());
        ...
        $this->assign('sum',$sum);
        $this->assign('real_sum',$real_sum['sum']==""?0:$real_sum['sum']);
        $this->assign('n_real_sum',$n_real_sum['sum']==""?0:$n_real_sum['sum']);
        $this->assign('list_data', $data);
        $this->meta_title = $model['title'].'列表';
        $this->display($model['template_list']);
    }
